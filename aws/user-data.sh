...
sed -i -r "s/.+(0.hdf.aws.mine)/$(ifconfig |grep 'addr:172' |awk '{print $2}' |cut -d ':' -f 2) \\1/" /etc/hosts
...
function waitAmbariServiceStart {
  ...
  while [ $COUNTER -lt 30 ]
  do
    STATE=$(curl -u admin:admin -s "http://localhost:8080/api/v1/clusters/hdf/services/${AMBARI_SERVICE}" |jq -r .ServiceInfo.state)
    if [ ${STATE} == "STARTED" ]
    then
...
function startAmbariService {
  ...
  do
    HTTP_RES=$(curl -u admin:admin -s -o /dev/null -H "X-Requested-By: ambari" -w "%{http_code}" -XPUT -d \
     ...
     "http://localhost:8080/api/v1/clusters/hdf/services/${AMBARI_SERVICE}")
...
for s in ZOOKEEPER AMBARI_INFRA AMBARI_METRICS NIFI KAFKA STORM
do
...
/opt/zeppelin-0.7.0-bin-netinst/bin/zeppelin-daemon.sh start
